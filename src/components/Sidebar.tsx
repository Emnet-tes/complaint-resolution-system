...
const Sidebar = ({ isMobileOpen, setIsMobileOpen }: SidebarProps) => {
  ...
  const allItems: SidebarItem[] = [
    // System Admin items
    { id: 'dashboard', label: 'System Overview', path: '/dashboard', roles: ['SysAdmin'], icon: LayoutDashboard },
    { id: 'organizations', label: 'Organizations', path: '/organizations', roles: ['SysAdmin'], icon: Building2 },
    
    // Org Admin items
    { id: 'org-dashboard', label: 'Dashboard', path: '/org-dashboard', roles: ['OrgAdmin'], icon: LayoutDashboard },
    { id: 'departments', label: 'Departments', path: '/departments', roles: ['OrgAdmin'], icon: ShieldCheck },
    
    // Shared items
    { id: 'complaints', label: 'Complaints', path: '/complaints', roles: ['OrgAdmin', 'DeptAdmin'], icon: ClipboardList },
    { id: 'settings', label: 'Profile', path: '/settings', roles: ['SysAdmin', 'OrgAdmin', 'DeptAdmin'], icon: SettingsIcon },
  ];
      ...
      >
          ...
          <div className="flex items-center overflow-hidden">
            ...
            <div className={`ml-3 transition-all duration-300 ${isCollapsed ? 'md:opacity-0 md:w-0' : 'opacity-100'}`}>
              <h2 className="text-[14px] font-black text-slate-800 whitespace-nowrap uppercase tracking-tight">
                {user?.role === 'SysAdmin' ? 'System Portal' : user?.role === 'OrgAdmin' ? 'Org Portal' : 'Dept Portal'}
              </h2>
          ...
          {filteredItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center py-3 transition-all rounded-xl group mb-1 ${
        ...
        {user && (
          <div className="p-4 mt-auto border-t border-gray-100">
            <button
              onClick={logout}
              className={`w-full flex items-center py-3 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors ${isCollapsed ? 'md:justify-center' : 'px-4'}`}
            >
              <LogOut size={20} className={!isCollapsed ? "mr-3" : ""} />
              <span className={`text-[13px] font-bold uppercase tracking-wider ${isCollapsed ? 'md:hidden' : 'block'}`}>Log Out</span>
            </button>
          </div>
        )}
      </aside>
