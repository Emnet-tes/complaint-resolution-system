...
const OrgComplaints = () => {
  ...
  const handleExport = () => {
    alert('Exporting org complaints as CSV...');
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <nav className="text-xs text-slate-400 mb-1">Dashboard / Org Complaints</nav>
          <h1 className="text-2xl font-bold text-slate-800">Organization Complaints</h1>
          <p className="text-sm text-slate-500">Manage complaints at the organization level.</p>
        </div>
      ...
      <div className="flex gap-3">
        <span className="bg-teal-50 border border-teal-100 text-[#006B5D] px-3 py-1 rounded-full text-xs font-bold flex items-center">
          Status: Open &amp; In Progress <X size={14} className="ml-2 cursor-pointer" />
        </span>
        <span className="bg-teal-50 border border-teal-100 text-[#006B5D] px-3 py-1 rounded-full text-xs font-bold flex items-center">
          Priority: Critical <X size={14} className="ml-2 cursor-pointer" />
        </span>
      ...
      {viewMode === 'list' ? (
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-slate-400 font-medium border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 uppercase text-[10px] tracking-wider">Org</th>
        ...
        <div className="bg-slate-200 h-96 rounded-2xl flex items-center justify-center border-4 border-dashed border-slate-300">
          <p className="text-slate-500 font-bold flex items-center">
            <MapIcon className="mr-2" /> Interactive Org Map View Initializing...
          </p>
        </div>
      )}

      <Modal isOpen={isAddComplaintOpen} onClose={() => setIsAddComplaintOpen(false)} title="File New Organization Complaint">
